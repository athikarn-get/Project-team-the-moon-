...
@onready var sprite_closed: Sprite2D = $Sprite2D
@onready var sprite_opened: Sprite2D = $Sprite2D2
...
func _ready() -> void:
	...
	if area == null:
		push_error("Chest3.gd: ต้องมี Area2D ชื่อ 'InteractionArea' ใต้ Chest")
		return
...
func _notify_main_chest_opened() -> void:
	...
	if main and main.has_method("on_chest_opened_with_hint"):
		main.on_chest_opened_with_hint("""💡 คำใบ้ข้อที่ 3
